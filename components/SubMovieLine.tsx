...
function SubMovieLine({ movie }: Props) {
  const router = useRouter();
  const [isHover, setIsHover] = useState(false);

  const navigatePage = async () => {
    if (movie.title) {
      router.push(`/details/movie${movie.title}`);
    } else {
      router.push(`/details?passTitle=${movie.title}`);
    }
  };
      ...
      {movie.movie_pics ? (
        ...
        >
          ...
          {isHover && (
            <>
              <motion.div
                className="absolute top-[110px] z-50 items-center ml-6 w-[180px]"
                initial={{ opacity: 0, scale: 0.5 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{
                  duration: 0.8,
              ...
              >
                ...
                <p className="text-xs">{"2024-04-19"}</p>
